...
angular.module('mainApp').controller('watchesCtrl',
['$scope', '$state', '$stateParams', '$rootScope', '$ionicScrollDelegate', 'watchesService',
	function ($scope, $state, $stateParams, $rootScope, $ionicScrollDelegate, watchesService) {
	    ...
	    var loadTotalProducts = function (searchtext, minPrice, maxPrice) {
	        ...
	        watchesService.getTotalProducts().get(form).$promise.then(
                function (response) {
                    // success
	    ...
	    var loadProducts = function (searchtext, orderby, minPrice, maxPrice, page) {
	        ...
	        watchesService.getProductDataBySearch().get(form).$promise.then(
                function (response) {
                    // success
                    $scope.loading = false;
                    if (response.data.length > 0) {
                        $scope.noResult = false;
                        var metaTitle = 'Online Shopping Hub | Watches ' + $stateParams.title;
