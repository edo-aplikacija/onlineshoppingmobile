...
angular.module('mainApp').controller('petSuppliesCtrl',
['$scope', '$state', '$stateParams', '$rootScope', '$ionicScrollDelegate', 'petSuppliesService',
	function ($scope, $state, $stateParams, $rootScope, $ionicScrollDelegate, petSuppliesService) {
	    ...
	    var loadTotalProducts = function (searchtext, minPrice, maxPrice) {
	        ...
	        petSuppliesService.getTotalProducts().get(form).$promise.then(
                function (response) {
                    // success
	    ...
	    var loadProducts = function (searchtext, orderby, minPrice, maxPrice, page) {
	        ...
	        petSuppliesService.getProductDataBySearch().get(form).$promise.then(
                function (response) {
                    // success
                    $scope.loading = false;
                    if (response.data.length > 0) {
                        $scope.noResult = false;
                        var metaTitle = 'Online Shopping Hub | Pet Supplies ' + $stateParams.title;
